...
/*
OWEN Next time you do this do not have multiple functions like moveClaw,moveClaws just have moveClaw(int speed); and moveClaws(int speed,int dist);
*/
...
//		playSoundFile("soviet.wav");
}

task autonomous()
{
	//Here you can put your autons
	if(auton == 0){
		competitionAuton();
	}
	else if(auton == 1){
...
//OK this comp auton doesn't work never tested it
//Because ;( provincials dreams were crushed
void goodCompetitionAutonL(){
...
//Very important functions allows me to move,lift and open claw at the same time
//Great
//Probably should document this but no

//Structure:
//While the lift is still under this height (so it is the last condition)
//drive until I reach my endpoint
//while driving if I touch a specified point start lifting the lift but keep driving
//when the lift hits a certain height start opening the claws until two potentiometer points
...
void moveClaws(int speed,int distL,int distR){
	//This function is actually crazy
	//tried to do somethings don't know if they worked, scared to change before competition
	int oldDistR[11];
	int oldDistL[11];
	oldDistR[10] = 50;
	oldDistL[10] = 50;
...
void debugStreamSensors(){
	wait1Msec(1000);
	//Print battery level in milliAmperes
...
task usercontrol()
{
	...
	while (true)
	{
		...
		//OMFG I LEFT THESE IN
		//Looking back at my code is giving me mini heart attacks
		//Tarj Goddamnit I am sorry
		if(vexRT[Btn8L]){
			drivePID(80,430);
		}
		if(vexRT[Btn8U]){
		...
		if(vexRT[Btn5U]){
			...
			//LAZY lift PID
			//SHAIV said not to bother with proper one for Quad Encoders
			if(SensorValue[Lift] < 650){
				motor[LiftL] = 15;
				motor[LiftL2] = 15;
				motor[LiftR] = 15;
...
//DON'T LOOK AT ME
//I AM LAZY OK
//3 FUNCTIONS to set 4 motor speeds
void moveForward(int speedL,int speedR){
	lDriveMove(speedL);
	rDriveMove(speedR);
}
...
//HAHAHHA moveLift PID yeah no just move lift until distance
void moveLiftPID(short speed,short dist){
	...
	//Lift "PID" really I just figured out that a motor speed of 15 kept the lift stable with any load! Incredible we didn't have any potentiometers
	//Shaiv said not to attempt LiftPID with quad encoders
	if(SensorValue[Lift] > 650){
		moveLift(-15);
	}
	else{
...
//Jesus christ this pid
//IDFK
//But calculations are cool will use next time
...
//Turn code really basic just move until the sensor values are correct
//Turning PID???? Is that possible?? It must be
void turn(int distL,int distR){
	SensorValue[LDrive] = 0;
	SensorValue[RDrive] = 0;
